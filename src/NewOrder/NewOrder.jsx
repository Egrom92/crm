            ...
            <div className="d-flex flex-column justify-content-center">
                <div className="card" style={{'width': 500}}>
                    ...
                    <div className="card-body">
                        <div className="form-group row">
                            ...
                            <div className="col-sm-9">
                                <input type="text" className="form-control" value={client} onInput={changeClient}/>
                            </div>
                        </div>
                        <div className="form-group row">
                            <label className="col-sm-3 col-form-label">Заказ:</label>
                            <div className="col-sm-9">
                                <select className="form-control" value={activeProduct} onChange={changeActiveProduct}>
                                    {/*<option key={0} value={'default'}>Выберите заказ</option>*/}
                                    {/*{products.map((product, i) => (*/}
                                    {/*    <option key={i+1} value={product.value}>{product.name}</option>*/}
                                    {/*))}*/}
                                </select>
                            </div>
                        </div>
                    ...
                    <div className="card-footer d-flex justify-content-end">
                        <button className="btn btn-success" onClick={() => sendNewOrder()}>Отправить</button>
                    </div>
                </div>
            </div>
        </div>
